...
// Дефолты из метаданных входа (Спринт 2): pad берёт размер входа, fps — частоту входа.
describe("pad.defaultsFromInfo — размеры из входа", () => {
  it("width/height = разрешение входа", () => {
    expect(pad.defaultsFromInfo!(info(1920, 1080))).toEqual({ width: 1920, height: 1080 });
  });

  it("без размеров входа (null) → пусто (остаётся статичный дефолт)", () => {
    expect(pad.defaultsFromInfo!({} as MediaInfo)).toEqual({});
  });
});

describe("fps.defaultsFromInfo — частота из входа", () => {
  it("value = fps входа, округлённый (29.97 → 30)", () => {
    expect(fps.defaultsFromInfo!({ fps: 29.97 } as MediaInfo)).toEqual({ value: 30 });
  });

  it("без fps входа (null) → пусто (остаётся статичный 30)", () => {
    expect(fps.defaultsFromInfo!({} as MediaInfo)).toEqual({});
  });

  it("fps = 0 (VFR/битый контейнер) → пусто (не даём невалидный fps=0)", () => {
    expect(fps.defaultsFromInfo!({ fps: 0 } as MediaInfo)).toEqual({});
  });
});
