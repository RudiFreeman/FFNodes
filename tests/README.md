...
| Генератор: цепочки по связям, порядок, разрыв, цикл, пустой граф, outputArgs, комбинация, путь, -af + -vf вместе; filter_complex путь (GIF-палитра, путь из params.path); мульти-аутпут (Спринт 3): несколько выходов→подсказка «в разработке» (пункт 1) | `src/shared/lib/ffmpeg/generate.test.ts` | 17 |
