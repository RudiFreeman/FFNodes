...
- CI-workflow `.github/workflows/release.yml` (сборка бандлов под 3 ОС): юнит-тестами не
  покрывается (нет среды) — **проверка = реальный прогон Actions**. Порядок: сперва ручной
  `workflow_dispatch` с тестовым тегом (все 4 job'а зелёные, ассеты в черновом релизе), затем
  боевой тег. Локально — только вычитка/линт YAML (`actionlint`, если установлен).
